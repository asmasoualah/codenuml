...
let updateValEnv id vEnv t = 
  let u = getU t in
  let p = getP t in
  let s = String.sub id 0 3 in
  let v = if ((String.compare s "+__")=0) then newPlusVar u p else
          if ((String.compare s "-__")=0) then newMinusVar u p else
          if ((String.compare s "*__")=0) then newMultVar u p else
          if ((String.compare s "/__")=0) then newDivVar u p else 
          if ((String.compare s "<F_")=0) then newLtFloatType 2 u p else 
...
| Nil -> true
| Plus(e1,e2,_,_) -> ((doesNotOccurs v e1) && (doesNotOccurs v e2))
| Minus(e1,e2,_,_) -> ((doesNotOccurs v e1) && (doesNotOccurs v e2))
| Mult(e1,e2,_,_) -> ((doesNotOccurs v e1) && (doesNotOccurs v e2))
| Div(e1,e2,_,_) -> ((doesNotOccurs v e1) && (doesNotOccurs v e2))
       ...
       (Int(s2'),Int(u2'),Int(p2')) -> 
         ...
         in
            if (p>0) then 
              ...
              raise (Error ("Type "^(printExpr (TFloat(s1,u1,p1)))^" is not compatible with type "^(printExpr (TFloat(s2,u2,p2))) ) )
...
| (TypeVar(rs,ss),TypeVar(ru,su),TypeVar(rp,sp)) -> 
    ((match !rs with
        None -> rs := Some(fst3 (copyType !s2 [] [] []))
      | Some(s1) -> unify s1 !s2
     ) ;
     (match !ru with
        None -> ru := Some(fst3 (copyType !u2 [] [] []))
      | Some(u1) -> unify u1 !u2
     ) ; 
     (match !rp with
        None -> rp := Some(fst3 (copyType !p2 [] [] []))
      | Some(p1) -> unify p1 !p2
     ) 
) 
...
and unifyVar t r name = match !r with
  None -> (match t with
             TypeVar(r',name') ->(*  if (not ((String.compare name name')=0)) then*) r := Some(t) (* modif du 10/11/16 !!!!!!!!*)
...
and typeCheck e env valEnv = match e with
  Float(f,u,p) -> let s = if ((F.to_float f)>0.0) then 1 else
