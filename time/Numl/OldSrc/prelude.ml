...
let rec ufp0 x i = if (x>(pow 2.0 i)) then (i+1) else ufp0 x (i-1);;
...
(*
let fabs f = if f < 0.0 then (f *. (-1.0)) else f
;;


let my_f_of_float_prec ff p = 
                  let si = ff < 0.0 in
                  let f = F.from_float (fabs ff) in
                  let (s',i') = F.to_string_exp_base_digits 2 p f in
                  let s'' = s'^(String.make p '0') in
                  let ent = String.sub s'' 0 i' in
                  let frac = String.sub s'' i' (p-i') in
                  let fent = ent_of_string ent (F.from_float 0.0) in
                  let ffrac = frac_of_string (rev frac) (F.from_float 0.0) in
                  let res = F.add fent ffrac in      
                    if si then F.mul res (F.from_float (-1.0)) else res ;;

let my_f_of_string_prec s p = 
                  F.from_string s ;;
