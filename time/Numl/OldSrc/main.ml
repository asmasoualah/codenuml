...
| (Float(f1,i1,j1),Float(f2,i2,j2)) -> (f1=f2) && (i1=i2) && (j1=j2)
| (Int(i1),Int(i2)) -> i1=i2
...
let rec recTypeCheck id e typeEnv varEnv = 
  ...
  in 
    ...
    with RecTypeError(tf,s) -> typeCheck e (setEnv id tf typeEnv) varEnv 
      ...
      while !continue do
        ...
        try
          let c = Parser.program Lexer.token lexbuf in
            (match c with
               TopExpr(e) -> let typeEnv' = copyTypeEnv !typeEnv [] in
			     let t = typeCheck e typeEnv' varEnv in
                             let t' = evalPart t in
