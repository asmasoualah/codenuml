...
| Cdr(e1) -> (occurs x e1)
| Nil -> false
| Plus(e1,e2,_,_) -> (occurs x e1)||(occurs x e2)
| Minus(e1,e2,_,_) -> (occurs x e1)||(occurs x e2)
| Mult(e1,e2,_,_) -> (occurs x e1)||(occurs x e2)
| Div(e1,e2,_,_) -> (occurs x e1)||(occurs x e2)
...
let rec printExpression e env = match e with
  Float(f,u,p) -> let ff = (F.to_float f) in 
                  let uf = ufp ff 
                  in if ((u-p+1)>uf) then 
...
| Car(e1) -> let (s1,env1) = printExpression e1 env in ("car "^s1,env1)
| Cdr(e1) -> let (s1,env1) = printExpression e1 env in ("cdr "^s1,env1)
| Nil -> ("[]",env)
| Plus(e1,e2,_,_) -> let (s1,env1) = printExpression e1 env in
                 let (s2,env2) = printExpression e2 env1 in
                   ("("^s1^" + "^s2^")",env2)
| Minus(e1,e2,_,_) -> let (s1,env1) = printExpression e1 env in
                  let (s2,env2) = printExpression e2 env1 in
                    ("("^s1^" - "^s2^")",env2)
| Mult(e1,e2,_,_) -> let (s1,env1) = printExpression e1 env in
                 let (s2,env2) = printExpression e2 env1 in
                   ("("^s1^" * "^s2^")",env2)
| Div(e1,e2,_,_) -> let (s1,env1) = printExpression e1 env in
                let (s2,env2) = printExpression e2 env1 in
                  ("("^s1^" / "^s2^")",env2)
...
| TFloat(s,u,p) -> let (s0,env0) = (match !s with
                                    ...
                                    | Int(i) -> if (i=(-1)) then ("-",env) else
                                                ...
                                                if (i=0) then ("0",env) else ("T",env) 
                                    | _ -> if !verbose then printExpression !u env else ("<expr>",env)
                                   ) 
                   in
                   let (s1,env1) = (match !u with
                                      TypeVar(_) -> printExpression !u env
                                    | Int(_) -> printExpression !u env
                                    | _ -> if !verbose then printExpression !u env else ("<expr>",env)
                                   ) 
                   in
...
let printExpr2 t typ = 
  match (t,typ) with
    (Float(f,_,_),TFloat(s,u,p)) -> (match (!u,!p) with
                                       (Int(u'),Int(p')) -> let _ = prettySymb := 0 in fst (printExpression (Float(f,u',p')) [])
                                     | _ -> raise (Error "unexpected expression in printExpr for float type")
                                  )
  | (Float(_),_) -> raise (Error "impossible in printExpr (float)")
