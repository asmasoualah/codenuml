...
| Iota(e1,e2) -> let s1 = expr2Z3 e1 in
                 let s2 = expr2Z3 e2 
                 in "(ite (= "^s1^" "^s2^") 1 0)"
| SigmaPlus(e1,e2) -> "1" (*let s1 = expr2Z3 e1 in
                      let s2 = expr2Z3 e2 
                      in ("(ite (= "^s1^" 0) 0
                           (ite (= "^s2^" 0) 0 
                           (ite (and (= "^s1^" 1) (= "^s2^" -1)) 0 
                           (ite (and (= "^s1^" -1) (= "^s2^" 1)) 0 
                           1))))")  *) 
| SigmaMinus(e1,e2) -> "1" (*let s1 = expr2Z3 e1 in
                       let s2 = expr2Z3 e2 
                       in ("(ite (= "^s1^" 0) 0
                            (ite (= "^s2^" 0) 0
                            (ite (and (= "^s1^" 1) (= "^s2^" 1)) 0 
...
let solve e1 v1 e2 v2 e3 v3 = 
  ...
  let v3' = evalPart v3 in
(*
let _ = err ("\n"^(printExpr e1')^" == "^(printExpr v1')) in
let _ = err ("\n"^(printExpr e2')^" == "^(printExpr v2')) in
let _ = err ("\n"^(printExpr e3')^" == "^(printExpr v3')) in
  *)
  let (lTV1,ls1) = getVars e1' [] [] in
  let (lTV2,ls2) = getVars v1' lTV1 ls1 in
  let (lTV3,ls3) = getVars e2' lTV2 ls2 in
  ...
  let a3 = ("(assert (>= "^lhs3^" "^rhs3^"))\n") in 
  let z3 = decl(*^a1*)^a2^a3^runZ3 in
  let s = "numl"^(newExprVar ())^".z3" in
  let f = open_out s in 
  (*let f = open_out ("numl.z3") in *)
  let _ = output_string f z3 in
  let _ = close_out f in 
  let sh = "z3 "^s^" > numl.res" in 
  let _ = Sys.command sh in 
  let _ = codeLine := 0 in 
  let f = open_in "numl.res" in
  let lexbuf = Lexing.from_channel f in 
