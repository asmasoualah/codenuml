...
| Nil -> e
| Plus(e1,e2,u,p) -> Plus(subst x e0 e1,subst x e0 e2,u,p)
| Minus(e1,e2,u,p) -> Minus(subst x e0 e1,subst x e0 e2,u,p)
| Mult(e1,e2,u,p) -> Mult(subst x e0 e1,subst x e0 e2,u,p)
| Div(e1,e2,u,p) -> Div(subst x e0 e1,subst x e0 e2,u,p)
...
| Iota(e1,e2) -> Iota(subst x e0 e1,subst x e0 e2)
| SigPlus(e1,e2,e3,e4) -> SigPlus(subst x e0 e1,subst x e0 e2,subst x e0 e3,subst x e0 e4)
| SigMinus(e1,e2,e3,e4) -> SigMinus(subst x e0 e1,subst x e0 e2,subst x e0 e3,subst x e0 e4)
| SigMult(e1,e2) -> SigMult(subst x e0 e1,subst x e0 e2)
| SigmaPlus(e1,e2) -> SigmaPlus(subst x e0 e1,subst x e0 e2)
| SigmaMinus(e1,e2) -> SigmaMinus(subst x e0 e1,subst x e0 e2)
| TInt -> e
| TBool -> e
| TFloat(r0,r1,r2) -> let r0' = subst x e0 !r0 in
                      let r1' = subst x e0 !r1 in 
                      let r2' = subst x e0 !r2 in TFloat(ref r0',ref r1',ref r2')
| TypeVar(r,s) -> (match !r with
                     None -> TypeVar(r,s)  
                   | Some e1 -> let _ = r := Some (subst x e0 e1) 
                                in TypeVar(r,s)  
                  )
| TProduct(e1,e2) -> TProduct(subst x e0 e1,subst x e0 e2)
| TList(e1) -> TList(subst x e0 e1)
;;
...
| Nil -> Nil
| Plus(e1,e2,u,p) -> let v1 = eval e1 env in
                     let v2 = eval e2 env in Float(F.add_prec (p+2) (getFP v1)  (getFP v2),u,p)
| Minus(e1,e2,u,p) -> let v1 = eval e1 env in
                      let v2 = eval e2 env in Float(F.sub_prec (p+2) (getFP v1) (getFP v2),u,p)
| Mult(e1,e2,u,p) -> let v1 = eval e1 env in
                     let v2 = eval e2 env in Float(F.mul_prec (p+2) (getFP v1) (getFP v2),u,p)
| Div(e1,e2,u,p) -> let v1 = eval e1 env in
                    let v2 = eval e2 env in Float(F.div_prec (p+2) (getFP v1) (getFP v2),u,p)
...
| BLt(e1,e2) -> let v1 = eval e1 env in
                let v2 = eval e2 env in 
                  (match (v1,v2) with
                     (Float(x1,_,_),Float(x2,_,_)) -> Bool(x1 < x2)
...
| FEq(e1,e2,u,p) -> let v1 = eval e1 env in
                    let v2 = eval e2 env in 
                      (match (v1,v2) with
                         (Float(x1,_,_),Float(x2,_,_)) -> Bool((F.compare x1 x2)=0)
...
| BEq(e1,e2) ->  let b1 = evalPart e1 in
                 let b2 = evalPart e2 in 
                    (match (b1,b2) with
                     ...
                     | (Float(j1,_,_),Float(j2,_,_)) -> Bool(j1 = j2)
                     | _ -> BEq(b1,b2)
                    )
...
let assignTV x v = match x with
  TypeVar(r,s) -> r := v
| _ -> raise (Error "TypeVar expexted in assignTV")
;;


let rec solveAux e x v a b = 
  if (a<b) then 
    let _ = assignTV x (Some(Int(a))) in 
    let res = eval (IMinus(e,v)) [] 
    in (match res with
          Int(i) -> if (i=0) then true
	  	            else solveAux e x v (a+1) b
   	    | _ -> raise (Error "unexpected case in Solve")
       )
  else false
;;

let eqCst e1 e2 = match (e1,e2) with
  (Int(a),Int(b)) -> a = b
| _ -> false ;;



let rec solveManyVar e v varList = match varList with
  [] -> if (e=v) then true 
        else (match (e,v) with
	        (Int(a),Int(b)) -> if (a=b) then true else false
              | _ -> raise SolveError
             )
| x::[] -> if (solveAux e x v (-500) 500) then true else (assignTV x None; false)
| x::xs -> if (iterSolve e v x (-500) 500 xs) then true else (assignTV x None; false) 


and iterSolve e v x a b xs =
  if (a<b) then
    let _ = assignTV x (Some(Int(a))) 
    in if (solveManyVar e v xs) then true else iterSolve e v x (a+1) b xs
  else
    (assignTV x None; false)
;;


let solve1 e v = 
(*  let _ = print_string ("\n solving "^(printExpr e)^" == "^(printExpr v)) in
  let _ = flush stdout in *)
  let (xs,_) = getVars e [] [] in
  let r = solveManyVar e v xs in
    if r then ()
    else raise (Error "incompatible constraints to solve, no solution found") ;;
...
let solve1 e v = 
  ...
  in (match xs with
        [] -> if (e=v) then () else 
		        (match (e,v) with
				   (Int(a),Int(b)) -> if (a=b) then () else raise (Error "incompatible constraints to solve, no solution found")
                                 | _ -> raise SolveError
	                )
      | x::[] -> if (solveAux e x v (-500) 500) then () else assignTV x None 
      | _ -> raise (Error "too many var in equation for the solver")
     ) 
;;
*)
let solve e v = 
...
| SigPlus(e1,e2,e3,e4) -> let _ = z32expr e1 in
                          let _ = z32expr e2 in
                          let e3' = z32expr e3 in
                          let e4' = z32expr e4 in ()
...
(*
let solveOneEq e v =
let _ = print_string ("\nsolving "^(printExpr e)^" = "^(printExpr v)^"\n") in
  let e' = evalPart e in
  let evl = getVars e' [] [] in
  let evl' = List.map removePrime (snd evl) in
  let evlDecl = vl2Z3 evl' in
  let ez3 = expr2Z3 e' in
  
  let v' = evalPart v in
  let vvl = getVars v' [] [] in
  let vvl' = List.map removePrime (snd vvl) in
  let vvlDecl = vl2Z3 vvl' in
  let vz3 = expr2Z3 v' in

  let runZ3 = "(check-sat)\n(get-model)\n" in 
  let a = ("(assert (= "^ez3^" "^vz3^"))\n") in 
  let z3 = evlDecl^vvlDecl^a^runZ3 in 
