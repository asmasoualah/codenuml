...
| Div(e1,e2,_,_,_) -> let (s1,env1) = printExpression e1 env in
                      let (s2,env2) = printExpression e2 env1 in
                        ("("^s1^" / "^s2^")",env2)
| Sqrt(e1,_,_,_)  -> let(s1,env1)=printExpression e1 env in ("sqrt"^s1,env1) 
(*| Sqrt(e1,_,_,_)  ->let (s1,env1)=printExpression e1 env in *)
...
| TFloat(s,u,p) -> let (s0,env0) = (match !s with
                                    ...
                                    | _ -> if !verbose then printExpression !s env else ("<expr>",env)
                                   ) 
                   in
